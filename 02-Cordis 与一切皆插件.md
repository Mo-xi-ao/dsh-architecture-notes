...
一切皆插件，不表示任意代码都能自动进入系统。插件仍然要经过 Profile、Bundle 或 Loader 配置接纳；Client Remote 也需要应用组合者显式 mount。DSH 的取舍是：运行时内部可替换，系统入口处可审查。

判断一个插件是否完整，可以连续追问：它注册了什么？属于哪个 Context？监听何时生效？卸载由谁撤销？如果只回答“它导出了一个类”，还没有回答生命周期问题。
